...
namespace ProjekcikASP.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var rentHistories = db.User.Find(userId).RentHistory;
         
            ViewBag.rentHistory = rentHistories;
            return View(model);
        }
        ...
        public ActionResult AddBalance(int cash)
        {
            var userId = User.Identity.GetUserId();
            var user = db.User.Find(userId);
            var x = Convert.ToInt32(user.PhoneNumber);
            x += cash;
            user.PhoneNumber = x.ToString();
